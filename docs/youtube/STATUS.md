...
- 第1話のショート動画：**完成・チャットで送付済み**（縦長1080×1920、63秒）。大家さんが佐野さんの失踪を語る場面〜「次はあなたの番です」〜ラストの「コトン」を切り出し。Remotionの`Yubinuke01Short`コンポジション（`docs/horror-scripts/remotion/01-yubinuke/src/ShortVideo.jsx`、`build_short.py`）
