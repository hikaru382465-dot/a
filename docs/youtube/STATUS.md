...
- ひかるが`openverse.org`・`wikimedia.org`も追加（2026-09-26）。ただしどちらも「本体のドメイン」は繋がるが、実際に検索・画像取得に使うサブドメイン（`api.openverse.org`、`commons.wikimedia.org`）は別扱いでブロックされたまま
  - サブドメインを1個ずつ追加すると切りがないので、**`*.openverse.org`・`*.wikimedia.org`のワイルドカードで登録し直すことを提案中**（ひかるの返答待ち）
